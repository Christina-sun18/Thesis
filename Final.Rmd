...
\end{equation}
  3.	Produce the one-step-ahead predictive $P^{t-1}_{\boldsymbol{\hat \theta_1^{[i]}}}$, and compute the out-of-sample score using $S_j$, where $S_i$ and $S_j$ refer to $\eqref{eq:LS}$ $\eqref{eq:CRPS}$ $\eqref{eq:FSR}$:^[The CRPS in $\eqref{eq:CRPS}$ is transformed to a positively-oriented score for the purpose of convenient comparison among all scores.]
