...
where $\theta_0$ is the true parameter and T is the total sample size of a time series variable $y_t$. The predictions conditional on this optimal estimator, $p(y_{T+1}|M,\hat \theta_n,y_{1:T})$, is said to be produced from a model that is 'optimized' according to a proper scoring rule. ^[M refers to the assumed model and it is omitted in the following notation for simplicity, since the assumed model does not change.]

In addition, in terms of evaluating predictions, it is usually the average score ($\bar S_n$) that is used to directly compare predictive accuracy among predictions produced by optimizing different scoring rules (Gneiting & Reftery, 2007). 
\begin{equation} \label{eq:sbar}
\bar S_n = \frac{1}{T}\sum_{t=1}^TS_n(p_{t+1},y_t)
\end{equation}
...
\textbf{True DGP} & $y_t = \sigma_t \epsilon_t$ & Stochastic volatility inversion copula \\
...
Copulas “reweight” the marginal densities to produce a joint density that captures the dependence among random variables, which can be applied to conditional distributions for forecasting purposes as shown in $\eqref{eq:copula}$. 
\begin{equation} \label{eq:copula}
H_t(x,y|\mathcal{F}_{t-1})=C_t(F_t(x|\mathcal{F}_{t-1}),G_t(y|\mathcal{F}_{t-1})|\mathcal{F}_{t-1})
\end{equation}
where $X_t$ and $Y_t$ are random variables with conditional marginal densities denoted by $F_t$ and $G_t$, $\mathcal{F}_t$ is an information set available at time t, and $H_t$ denotes the conditional joint distribution of $X_t$ and $Y_t$ [@fan2014]. 

The stochastic volatility inversion copula model used in this section allows us to keep the dependence structure of a state space model and provides the flexibility of using an asymmetric marginal distribution that is often observed in financial returns but is not allowed if we use state space models alone [@inversion2018].
...
Building on the previous research in this field, we conduct a numerical analysis in this section to further investigate the effects of the form and degree of model misspecification on the performance of optimal forecasts. 
...
We still use ARCH(1) model in Section 3 as the assumed predictive model since it is a reasonable model to predict stock returns and volatility. More importantly, it is misspecified with a high degree of model misspecification. Therefore, based on the results shown in Section 3, we should expect that predictions produced by optimizing CLS do a better job in predicting two tails of the distribution. The results of Table 8 support our conjecture. However, the difference between using CLS10(80) and CLS20(90) is very small. 
