...
This research paper will explore the question: ‘When do optimal probabilistic forecasts work’; with particular attention given to the usefulness of optimal predictions in financial markets. This work builds on recent research on this topic by Loaiza-Maya et al. (2020). Probabilistic forecasts can provide complete information about future uncertainty, which could be more valuable than point and interval forecasts to forecasters. However, both Bayesian and frequentist prediction assume that the underlying model correctly specifies the process that has generated the observations. While in practice, the assumed model underpinning the likelihood function will almost certainly differ from the unknown true data generation process (DGP). This model misspecification problem has been an ongoing issue for conventional likelihood-based prediction. 

As an alternative to likelihood-based prediction, scoring rules have been introduced before to produce and evaluate probabilistic forecasts. A variety of proper scoring rules allows users to produce probability predictions based on their interests, and to compare these predictions, as long as they refer to the same forecast quantity (Gneiting & Raftery, 2007). Some recent and previous researches show that forecasts produced based on a given scoring rule can be improved for both Bayesian and frequentist prediction, even in the presence of model misspecification; see Loaiza-Maya, Martin & Frazier (2019), Loaiza-Maya et al. (2020) and Opschoor, van Dijk & van der Wel (2017). Among all these, censored likelihood score (CSL) or focused score (FSR) proposed by Diks, Panchenko & van Dijk (2011), captures our attention since its ‘focusing’ feature shows great values in financial risk management. In other words, CSL/FSR allows a forecaster to focus on predicting any region of his/her interests more accurately and therefore, it is expected to make great contributions to risk estimation in finance. 

The ‘optimal’ probabilistic forecast has been discussed in Loaiza-Maya et al. (2020). It refers to a predictive probability distribution that is optimal according to a user-specified scoring rule. However, the discussion of whether the optimal forecasts’ performance differs among different types of model misspecification is left to blank, and the financial application of optimal forecasts can be further investigated among different financial instruments.

This research paper extends the work of Loaiza-Maya et al. (2020) to investigate the effects of the form and degree of model misspecification on optimal forecast performance and to conduct empirical analysis on how optimal forecasts perform in Value-at-Risk (VaR), the Volatility Index (VIX) and portfolio optimization applications, using empirical data extending over a period of time that precedes the global financial crisis (GFC) and extends to the latest period in which COVID-19 has impacted on financial markets. 

Section 2 documents how scoring rules work in producing and evaluating the optimal forecasts under both simulation and empirical settings. The background and notation of scoring rules used in this paper are provided at the start. Besides, the simulation analysis investigates the effects of different forms and degrees of model misspecification on optimal forecast performance using data simulated from the (generalized) autoregressive conditional heteroscedasticity ((G)ARCH) and copula models. The assumed predictive model is fixed as ARCH (1) for the purpose of manipulating the degree of model misspecification. The performance of optimal forecasts in practice is also shown in this section, conducted with S&P500 daily log return data. In section 3, we look at the financial application of scoring rules in VaR using both simulated and S&P500 observations with same methodology adopted in section 2. The analysis of VIX is provided in section 4, and the predictive model is HAR-RV model with different error term specifications. We conclude and discuss any possible caveats in section 5. 

# 2. Scoring rule in prediction
## 2.1 Overview and notation 

Scoring rules can be used in both estimation and prediction. In estimation, an optimum estimator $\hat \theta$ can be obtained by maximizing a scoring rule $S_n$ on the condition that $S_n$ is positively oriented. That is, $\hat \theta_n=arg \max_\theta S_n(\theta)$, $\hat \theta \to \theta$ as $n \to \infty$. While in prediction, scoring rules assign a numerical value to density forecasts, simultaneously addressing both sharpness and calibration (Gneiting & Reftery, 2007). For a positively oriented score, higher value will be assigned to a better forecast between two competing candidates. The crucial importance of the propreity of scoring rules must be emphasized for the purpose of ranking and comparing forecasts based on scoring rules. Gneiting & Raftery, (2007) illustrate the importance of propreity via a case study in weather forecasts and point out that improper scoring rules will assign higher average score to an incorrect density forecast and consequently, mess up the prediction evaluation. Suppose P and Q are predictive distributions and Q is the best forecast given all the available information. S(P,Q) denotes the expected value of S(P,$\cdot$) under Q. A scoring rule is said to be proper if $S(Q,Q) \geq S(P,Q)$ for all P and Q, and is strictly proper if $S(Q,Q) = S(P,Q)$ only happens when P = Q (Gneiting & Raftery, 2007). 

Some commonly used proper scoring rules are the logarithmic score (LS), the continuously ranked probability score (CRPS) and censored likelihood score (CSL) or focused score (FSR) in a sense that CSL focuses on predicting a certain region(s) of densities. 

The logarithmic score is defined as (1) where $p_t$ is the probabilistic density forecast. It is a local strictly proper scoring rule, which means it will assign a higher score to the correct probabilistic forecast, and it is superior to quadratic and spherical scoring rules when the rank ordering is important or the impact of nonlinear utility function used is a concern for forecasters (Bickel, 2007). 
\begin{align}
S_{LS}(p_t, y_{t+1}) = log[p_t(y_{t+1})]
\end{align}
However, logarithmic score is criticized by its unboundness and its local property. Bernardo (1979) states "locality requires the utility of probabilistic influence to depend only upon the probability density of the true state". Gneiting & Raftery (2007) also argue that logarithmic score is insensitive to distance and will not reward predictions that are close to but not identical to the materialized event. Therefore, Gneiting & Raftery, (2007) propose the continuously ranked probability score, which is sensitive to distance and is defined as in (2) where P is the cumulative distriubtion function, I is the indication function, and x is the materialized events. 
\begin{align}
...
CRPS is defined as a negatively oriented scoring rule, but it can be easily transformed to positively oriented form as shown in (4) for convenient comparison among scoring rules in following sections. 
\begin{align}
CRPS^*(N,x)=\sigma[-\frac{1}{\sqrt{\pi}}+2\phi(\frac{x-\mu}{\sigma})+\frac{x-\mu}{\sigma}(2\Phi(\frac{x-\mu}{\sigma})-1)]
\end{align}
Both LS and CRPS are used for producing and evaluating the entire predictive density. In terms of accurately predicting a certain region of a distribution, they are typically used together with weighted likelihood (Gneiting & Ranjan, 2011). Diks, Panchenko & van Dijk, (2011) propose a new proper scoring rule, censored likelihood score, which allows users to assess the forecasts only in the interested region(s) instead of using weights to emphasize a particular part of the entire density forecast. It is defined as in (5) where $p_t$ is the predictive probability density function, $A_t$ is the interested region(s) and $A_t^c$ is the complement of $A_t$.
\begin{align}
S_{CSL}(p_t, y_{t+1})=I(y_{t+1} \in A_t)log[ p_t(y_{t+1})]+I(y_{t+1} \in A_t^c)log[\int_{A_t^c} p_t(s)ds]
\end{align}
Opschoor, van Dijk & van der Wel (2017) prove that weighted density forecasts based on optimizing the censored likelihood score outperform those on LS and CRPS. 

## 2.2 Numerical investigation
### 2.2.1 Simulation design 

Loaiza-Maya, Martin & Frazier (2019) propose a new method, Focused Bayes, in Bayeisian prediction by replacing the conventional likelihood function with the censored likelihood score. In their simulated and empirical analysis, Focused Bayes outperforms exact Bayes which is using logarithmic score for updating the prior probability and they also point out the model misspecification plays a role in the performance of Focused Bayes since focusing incorrectly can harm. 

Loaiza-Maya et al.(2020) extend the above discussion to frequentist probability forecasting. They address questions about when we can/cannot benefit from optimal probabilistic forecasts out of sample. Loaiza-Maya et al.(2020) introduce the concepts of ‘coherence’ and ‘strict coherence’ for conveniently documenting the optimal forecasts’ performance. Coherence means when the optimal probabilistic forecast based on a given score is superior, or at least performs the same as, alternative forecasts according to the same score. Strict coherence is when the optimal prediction is strictly preferable given that score. Let $\boldsymbol{\hat \theta}_1$ and $\boldsymbol{\hat \theta}_2$ be the optimizers based on scoring rules: $S_1, S_2$, then the prediction is said to be coherent if 
\begin{align}
...
\end{align}
It is said to be strictly coherent if the equality doesn't hold in (6)(7). 

Building on the previous research in this field, we conduct a numerical analysis first in this sub section to further investigate the effects of the form and degree of model misspecification on optimal forecast performance. In order to prepare the financial application analysis in following sections of this paper, we simulate a time series dependnet variable $y_t$ that mimics the behavior of financial returns and volatility. Specifically, GARCH models are used to capture the volatility clustering and serial dependence usually observed in empirical stock return's distribution, and the negative marginal skewness will be incorporated by an inversion copula with degree of skewness controlled by the shape parameter. In order to manipulate the degree and form of model misspecification, we fix the underlying model to a simple and clearly misspecified ARCH(1) model.
...
\end{align}
  3.	Produce the one-step-ahead predictive $P^{t-1}_{\boldsymbol{\hat \theta_1^{[i]}}}$ And compute the out-of-sample score using $S_j$, where $S_i$ and $S_j$ refer to (10)(11)(12):
...
Moreover, the predictive density plots (Appendix 1) in figure 3 to 8, further support our conclusions drawn from the average score tables and score trace plots in above sections. They show that predictions obtained from optimizing focused scores aim to match the shape of tails of the true distribution and the changes over time/iterations. 
...
The data used in the analysis is the continuously compounded daily log returns of S&P500 listed in U.S.A financial market over a broad time period from 27 Sep,1996 to 30 July,2020. It aims to include two very volatile periods, 2008 Global financial crisis (GFC) and the recent COVID-19 pandemic. The methodology used here is the same as in the section 2.2 with initial training sample size set to be 1000 and 5000 out-of-sample observations. 
...
annotate_figure(figure, 
                ...
                fig.lab = "Figure 9",fig.lab.face = "bold"
                )
```

The descriptive statistics show that the distribution of S&P500 log returns are negatively skewed and have fat tails and strong serial correlation in its volatility. Besides, during the GFC and COVID-19 pandemic periods, the volatility is very high due to high uncertainty. 

We still use ARCH(1) model in section 2.2 as the assumed predictive model since it is a reasonable model to predict stock returns and volatility and more importantly, it is misspecified with a high degree of model misspecification. Therefore, we should expect that predictions produced by optimizing FSR do a better job in predicting two tails of the distribution. The results of Table 8 support our conjecture. However, the difference between using FSR10(80) and FSR20(90) is very small. 
...
sp_table %>% 
  ...
  kable_styling("bordered",full_width = F)%>%
  add_header_above(c("","Average out-of-sample scores"=6)) %>% 
  add_header_above(c("Table 8: Average scores for S&P500"=7))
```

Unlike simulation, the score trace plots show how different scoring rules perform in real time points which correspond to some historical events. In figure 10, one interesting phenomenon is that after the GFC, focused score performs much better than CRPS and MLE compared with before, in terms of predicting both upper and lower tails. And this 'jump' effect continues until the very recent date. It indicates that optimal probabilistic forecasts might predict financial markets better in and after times of stock market turbulance, which could represent a great contribution to the existing financial analyses of risks and returns. 
...
ggarrange(fsr10plot,fsr90plot,ncol=2,legend = "bottom",labels = c("Figure 10(a) Lower Tail","Figure 10(b) Upper Tail"))
...
Density forecasts have great practical values in finance. It forms the foundation of risk management, such as Value-at-Risk (VaR) and is useful for asset allocation and derivative pricing. VaR is a commonly used risk measurement in finance. It can be obtained from a probability density function of returns over a chosen investment horizon. It is the p-quantile of a predictive distribution of a portfolio’s profit/loss over a holding period, according to a given confidence interval $\alpha$ (Dowd, 2005).
\begin{align}
VaR = -q_p \\p=1-\alpha
\end{align}
It indicates that investors will not lose more than the amount as shown in VaR. It represents the worst level of the investments with the given $\alpha$ confidence level. There are different methods to estimate VaR, but it is clear that predictive density plays an important role here. Therefore, a more accurate prediction can contribute to improving the accuracy of VaR estimation. 

Opschoor, van Dijk & van der Wel (2017) conduct combining density forecasts using focused scoring rules, and the results show weighted density forecasts based on optimizing the focused score outperform those based on optimizing CRPS or LS and improve the accuracy of 99% Value-at-Risk estimates. 

We illustrate how optimal forecasts perform in estimating VaR by usig predictions produced in section 2.2 and 2.3 and expect optimal forecasts based on FSR would give us a better prediction in tails. We implement VaR analysis by using VaR backtesting method. Specifically, we follow the steps as bellow:

1. Using the predictive densities produced in section 2.2 and 2.3: $p(y_{1001}|y_{1000})...p(y_{5000}|y_{4999})$;

2. Construct the VaR at \{10%, 20%, 80%, 90%\} using predictive densities in step 1;

3. Compare the true values of $y_{1001}...y_{5000}$ with $VaR_t$ and calculate the proportion of exceedances ($y_t < VaR_t$); An accurate estimation of the p% VaR is observed if the proportion of exceedances equals to p%.

## 3.2 Value-at-Risk for simulation analysis

The following tables include VaR estimations at \{10%, 20%, 80%, 90%\} corresponding to the 10% and 20% expected loss of long and short portfolios. The values showed in these tables refer to the out-of-sample proportion of exceedances over the nominal VaR level indicated by the column name. The closer the proportion of exceedances to nominal VaR levels, the more accurate are VaR estimations, and thus the more accurate are the optimal probablistic forecasts. We can see that in two different forms of model misspecification displayed in Table 9 and 10, focused score can provide the most accurate VaR estimation in the upper tail, while in lower tail, it is not short of advantages by too much compared with CRPS. These results help us understand what to expect in empirical VaR analysis.
...
df %>% 
  ...
  add_header_above(c("Table 9: VaR when true DGP is copula (shape = -5)"=5))
```
...
df %>% 
  ...
  kable_styling("bordered",full_width = F) %>%
  add_header_above(c(" ","Out-of-sample exceedances"=4)) %>%
  add_header_above(c("Table 10: VaR when true DGP is GARCH with degree of freedom = 3"=5))
```

## 3.3 Value-at-Risk for empirical analysis

We now implement the VaR analysis to continuously compounded daily log returns of S&P500 in U.S.A financial market. Learning from score trace plots in section 2.3.1, the 2008 GFC is the turning point of the optimal forecasts performance, especially for focused scores. Therefore, we separate the VaR analysis to two different time periods, before GFC and after GFC. The results from Table 11 and 12 show that the focused score perform well enough in terms of estimating VaR for both long and short portfolios of S&P500, but also CRPS is a very robusted scoring rule which can provide relatively accurate predictions. Moreover, in Table 12, focused score seems to perform better in 20% and 80% which have higher probability mass. If we think of the marginal distribution of S&P500 estimated by histogram in section 2.3.1, the above results could be a consequence of the very sparsed marginal distribution, which increases the difficulty of 'focusing' on the 10% and 90% tails.
...
\end{align}
When considering the sudden jumps, that is, the price follows the process in (18). RV is also a consistent estimate of quadratic variation (QV) where $\kappa_s$ represents the sudden jump on day t and $q_t$ is the jump occasions.  
\begin{align}
...
VIX index estimates QV that is implied by option prices under a risk-neutral (18) process, using a finite number of strikes of S&P500 index (Chicago Board Options Exchange, 2020). It acknowledges the occurance of jumps and the fact that assumptions of Black-Scholes model do not hold. Since both VIX and RV are estimations of QV, it is reasonable to model VIX with a HAR-RV model which is usually used for modelling RV in the literature (Corsi, 2009; Maneesoonthorn, Martin, Forbes & Grose, 2012; Martin, Reidy & Wright, 2009; Andersen, Bollerslev & Diebold, 2007).

## 4.2 VIX descriptive statistics 
We collect daily VIX index data starting from 27 Aug,1996 to 30 July,2020. And the descriptive statistics of $log(VIX_t)$ are shown in Table 13 and Figure 11: 

Table 13 and Figure 11 shows that $log(VIX_t)$ is volatile and shows persistence over time. It is positively-skewed and the autocorrelation of its volatility is slowly decaying. From the descriptive statistics, we can see that the features of VIX is similar to what we have observed from S&P500. Similarly, there are unusual jumps happened during GFC and COVID-19. Thus, we expect that focused score could outperform MLE and CRPS in predicting tails as before and significantly improve accuracy after GFC. 
...
annotate_figure(figure, 
                top = text_grob("Descriptive figures",face = "bold"),
                fig.lab = "Figure 11",fig.lab.face = "bold"
                )
```

## 4.3 Model specification

Proposed by Corsi (2009), HAR-RV model is a simple additive linear model which takes lagged squared returns as regressors. It does not belong to the class of long memory models, but it is able to produce the volatility persistence that is almost indistinguishable from what observed in financial markets through the simple AR-type structure.  

We design the VIX analysis with different error term specification of HAR-RV: 
\begin{align}
...
2. $z_{t+1} \sim Student-t(\mu,\sigma,\nu)$ and constant $\sigma$ 
...
By using the same methodology in section 2 with initial sample size = 1000 and out-of-sample size $\tau=T-1000=5000$, we produce and evaluate predictive densities with the assumed model as HAR-RV.

### 4.3.1 Results of specification 1: Normal error
...
vix_table %>% 
  ...
  kable_styling("bordered",full_width = F)%>%
  add_header_above(c("","Average out-of-sample scores"=6)) %>% 
  add_header_above(c("Table 14: Average scores for VIX"=7))
```
...
In Table 14, as expected, FSR performs better in predicting upper tail as expected and does a relatively good job in predicting lower tail as well, even though CRPS ranks the best in terms of forecasting 20 percentile of the distribution. The score trace plot, Figure 12, shows the prediction of optimizing four scoring rules evaluated by FSR10 (left panel) and FSR90 (right panel). It indicates how accurate these predictions according to their performance in 10 percentile and 90 percentile. In the lower tail, it seems that there is not much difference among four predictions, but if zoom in, we can see that FSR is at the top of other scores, which means it can still provide the most accurate prediction. However, we only gain very little from using it. On the contrary, in the upper tail, FSR has a much higher effect on improving accuracy after GFC and this effect continues. One possible explanation could be that both LS and CRPS can predict the lower tail better if there is higher probability mass at the area, then the benefit of using FSR is not significant. Considering the marginal distribution shown in figure 13.B of $log(VIX_t)$, we observe much more observations in the lower end than in the upper end so that it might be the reason why we observe the little improvement in the lower tail in Figure 12(a). 

### 4.3.2 Results of specification 2: Student-t
...
...
1. Optimal forecasts perform almost the same in two different types of model misspecification investigated in this paper and the more misspecified the model is, the more benefits can we gain from using focused score in improving the tail prediction accuracy. 

2. Focused score improves the prediction accuracy by a lot after the stock market turbulance and can give better Value-at-Risk estimation. 

3. In terms of modelling VIX index, the HAR model with normal error term shows that focused score really makes a difference in predicting upper tail but no great improvement observed in the lower end. 

Caveats: 

1. The VaR is conditional distribution while the focused score is unconditional distribution due to the computation burden.
