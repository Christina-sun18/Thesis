...
\begin{align}
S_{LS}(\hat f_t, y_{t+1}) = log \hat f_t(y_{t+1})
\end{align}
where $\hat f_t$ is the density forecasts based on available observations. 


$$S_{CSL}(\hat f_t, y_{t+1})=I(y_{t+1} \in A_t)log \hat f_t(y_{t+1})+I(y_{t+1} \in A_t^c)log[\int_{A_t^c}\hat f_t(s)ds]$$
