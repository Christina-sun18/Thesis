...
Value-at risk (VaR) is a commonly used risk measurement in finance. It can be obtained from a probability density function of returns over a chosen investment horizon. More formally, it is the p-quantile of a predictive distribution of a portfolio’s profit/loss over a holding period (Dowd, 2005).
\begin{align}
VaR = -q_p \\p=1-\alpha
\end{align}
It indicates that investors will not lose more than the amount as shown in VaR. It represents the worst level at a given level of confidence, $\alpha$. There are different methods to estimate VaR, but it is clear that predictive density plays an important role here. Therefore, a more accurate prediction would improve the accuracy of VaR estimation. 

We implement VaR analysis by using VaR backtesting method. It evaluates the prediction accuracy with the proportion of exceedances ($y_t < VaR_t$). That is, an accurate prediction of the $\alpha$% VaR is observed if $\alpha$% proportion of exceedances. We illustrate how optimal forecasts perform in estimating VaR by using simulation data from section 2 first. 

## 3.2 Value-at-Risk for simulation analysis

The following tables include VaR estimation at \{10%, 20%, 80%, 90%\} corresponding to the 10% and 20% loss of long and short portfolios.
```{r}
...
The results from Table 11 and 12 show that the focused score perform well enough in terms of forecasting the two tails of S&P500 log return distribution, but also CRPS is a very robusted scoring rule which can provide relatively accurate predictions. If we think of the marginal distribution of S&P500 estimated by histogram in section 2.3.1, the above results could be a consequence of less observations in two tails, which increases the difficulty of focusing on the tails.  
