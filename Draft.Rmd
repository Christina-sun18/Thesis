...
sum %>% 
  kable("latex",booktabs = T) %>% kable_styling("bordered")
```
...
```{r}
sum <- data.frame(Series="Log(VIX)",Min = 2.213, Median = 2.916, Max = 4.415,Skewness= vixsk,Kurtosis=vixkt,JB.Test=415.06,LB.Test=11744)
sum %>% 
  kable("latex",booktabs = T) %>% kable_styling("bordered")
```
