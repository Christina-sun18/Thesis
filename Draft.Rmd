...
\end{align}
  ...
  3.	Produce the one-step-ahead predictive $P^{t-1}_{\hat \theta_1^{[i]}}$ And compute the out-of-sample score using $S_j$, where $S_i$ and $S_j$ refer to (8)(9)(10):
  
\begin{align}
S_{LS}(P_\theta^{t-1},y_t)=lnp(y_t|\mathcal{F}_{t-1},\theta)
\end{align}

\begin{align}
S_{CRPS}^*(P_\theta^{t-1},y_t)=\int^{+\infty}_{-\infty}[P(y|\mathcal{F}_{t-1},\theta)-I(y \geq y_t)]^2dy
\end{align}

\begin{align}
S_{FSR}(P_\theta^{t-1},y_t)=lnp(y_t|\mathcal{F}_{t-1},\theta)I(y_t \in A)+[ln\int_{A^c}{p(y|\mathcal{F}_{t-1},\theta)dy}]I(y_t \in A^c)
\end{align}

4. Expand estimation window by one observation and repeat step 2-3 with $\tau = T-1000$ times and compute the average scores: 
\begin{align}
\bar S_j(\hat \theta^{[i]})=\frac{1}{\tau}\sum^T_{t=T-\tau+1}S_j(P_{\hat \theta^{[i]}}^{t-1},y_t)
\end{align}
...
It is said to be strictly coherent if the equality doesn't hold in (11)(12). 
...
Value-at-Risk (VaR) is a commonly used risk measurement in finance. It can be obtained from a probability density function of returns over a chosen investment horizon. It is the p-quantile of a predictive distribution of a portfolio’s profit/loss over a holding period, according to a given confidence interval $\alpha$ (Dowd, 2005).
\begin{align}
VaR = -q_p \\p=1-\alpha
\end{align}
It indicates that investors will not lose more than the amount as shown in VaR. It represents the worst level of the investments with $\alpha$ confidence level. There are different methods to estimate VaR, but it is clear that predictive density plays an important role here. Therefore, a more accurate prediction can contribute to improving the accuracy of VaR estimation. 

[ref 5] conduct combining density forecasts using focused scoring rules, and the results show weighted density forecasts based on optimizing the focused score outperform those based on optimizing CRPS or LS and improve the accuracy of 99% Value-at-Risk estimates. 

We implement VaR analysis by using VaR backtesting method. It evaluates the prediction accuracy with the proportion of exceedances ($y_t < VaR_t$). That is, an accurate estimation of the p% VaR is observed if the proportion of exceedances equals to p%. We illustrate how optimal forecasts perform in estimating VaR by using simulation data from section 2 first and expect outperformance of focused scoring rules. 

## 3.2 Value-at-Risk for simulation analysis

The following tables include VaR estimations at \{10%, 20%, 80%, 90%\} corresponding to the 10% and 20% loss of long and short portfolios.
```{r}
...
The values showed in these tables refer to the out-of-sample proportion of exceedances over the nominal VaR level indicated by the column name. The closer the proportion of exceedances to nominal VaR levels, the more accurate are VaR estimations, and thus the more accurate are the optimal probablistic forecasts. We can see that in two different forms of model misspecification displayed in Table 7 and 8, focused score can provide the most accurate VaR estimation in the upper tail, while in lower tail, it is not short of advantages by too much compared with CRPS. These results help us understand what to expect in empirical VaR analysis.
...
VIX index estimates QV that is implied by option prices under a risk-neutral (18) process, using a finite number of strikes of S&P500 index [reference from CBOE]. It acknowledges the fact that assumptions of Black-Scholes model do not hold and the occurance of jumps. Since both VIX and RV are estimations of QV, it is reasonable to model VIX with a HAR-RV model which is usually used for modelling RV in the literature. 

## 4.2 VIX descriptive statistics 

We collect daily VIX index data from (ref: CBOE) starting from 27 Aug, 1996 to 31 July, 2020. And the descriptive statistics of $log(VIX_t)$ is shown as below: 
...
By using the same methodology in section 2 with initial sample size $\tau = 1000$ and out-of-sample size $T-\tau+1=5000$, we produce the following average score table, Table ? and score trace plot, Figure ?. 
...
From Table ?, the results are not surprising focused score performs better in predicting upper tail as expected and does a relatively good job in predicting lower tail as well, even though CRPS ranks the best in terms of forecasting 20 percentile of the distribution. 

The score trace plot, Figure ?, shows the prediction of optimizing four scoring rules evaluated by FSR10 (left panel) and FSR90 (right panel). It indicates how accurate these predictions are in 10 percentile and 90 percentile. The scoring rules used for optimization are {MLE, CRPS, FSR10(90), FSR20(80)}. 

In the lower tail, it seems that there is not much difference among four predictions, but if zoom in, we can see that FSR is at the top of other scores, which means it can still provide the most accurate prediction. However, we only gain very little from using it. On the contrary, in the upper tail, FSR has a much higher effect on improving accuracy after GFC and this effect continues. 
One possible reason could be that considering the marginal distribution shown in figure 13.B of $log(VIX_t)$, both LS and CRPS can predict the lower tail better if there is higher probability mass at that end. Thus, the benefits of using FSR is not significant. 
...
2. The practical meaning of the VaR is more often focused on around 1%, while at this very small tail, the focused score gets less and less accurate since the observations are less, harder to focus. 
